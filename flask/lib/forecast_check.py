...
def check_rain_fall_openweathermap():
    ...
    rainfall_list = list(
        map(
            lambda x: [
                timezone("Asia/Tokyo")
                .localize(datetime.fromtimestamp(x["dt"]))
                .strftime("%c %z"),
                x["rain"]["3h"],
            ],
            filter(
                lambda x: (
                    (datetime.fromtimestamp(x["dt"]) - datetime.now()).total_seconds()
                    / (60 * 60)
                )
                < 8,
    ...
    with open(
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "forecast.log"),
        mode="a",
    ) as f:
        print(
            "{} {} => {}".format(
